...
import com.idolmedia.yzymanager.databinding.ItemCommodityManageBinding
 ...
 *  描述：
 */
class ItemCommodityManageBean( commodityItem : CommodityListEntity.Data, onItemDeleteListener: OnItemDeleteListener?) : ItemBaseManageBean(commodityItem,onItemDeleteListener) {

    init {
        layoutId = R.layout.item_commodity_manage
    }
    ...
    override fun refreshButton(){
        ...
        manageList.add(ItemManageButtonBean(commodityItem,this,if (isShelves.get()) "下架" else "上架"))
        manageList.add(ItemManageButtonBean(commodityItem,this,"调整库存"))
    ...
    override fun onBindViewModel(context: Context, binding: ViewDataBinding?, position: Int) {
        ...
        if (binding is ItemCommodityManageBinding){
